...
class UserModel:
    def __init__(self, bot: bot.SenvrBot, executor: concurrent.futures.ThreadPoolExecutor, ctx):
        ...
        self.ctx = ctx
        self.queue = asyncio.Queue(maxsize=int(os.getenv("ML_SAMPLE_SIZE")))
        self.channel_dict = {}
    ...
    async def make_sentence(self):
        await self.make_ready.wait()
        async with self.lock:
            if self.model:
                return await self.bot.loop.run_in_executor(self.executor, self.model.make_sentence)

    async def channel_crawler(self, channel: discord.TextChannel):
        try:
            async for message in channel.history(oldest_first=True, limit=None, after=self.channel_dict[channel]):
                if not self.crawl.is_set():
                    return logging.info(f"{self.ctx.author}: stopping now")
                else:
                    if message.author.id == self.ctx.author.id:
                        ...
                        if self.queue.qsize() >= int(os.getenv("ML_SAMPLE_SIZE")):
                            ...
                            async with self.lock:
                                ...
                                if self.model:
                                    logging.debug(f"{self.ctx.author.id}: Combining new model")
                                    self.model = await self.bot.loop.run_in_executor(self.executor, markovify.combine,
                                                                                     (self.model, new_model))
                                    if self.model.make_sentence():
                                        self.make_ready.set()
                                else:
                                    logging.debug(f"{self.ctx.author.id}: Setting new model")
                                    self.model = new_model
                            logging.info(f"{self.ctx.author.id}: Trained")
                        async with self.lock:
                            self.channel_dict[channel] = message
        ...
        for channel in self.ctx.guild.text_channels:
            async with self.lock:
                if channel not in self.channel_dict.keys():
                    self.channel_dict[channel] = None
            ...
            async with ctx.typing():
                async with self.lock:
                    if ctx.author.id in self.usermodel_dict.keys():
                        ...
                        result = await usermodel.make_sentence()
                        if not result:
                            result="Please wait a little longer"
